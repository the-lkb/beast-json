...
// NOTE: DOM parser uses a lenient digit scanner: any sequence starting
...
TEST(StrictNumber, ValidNumbersAccepted) {
  EXPECT_TRUE(dom_ok("0"));
  EXPECT_TRUE(dom_ok("123"));
  EXPECT_TRUE(dom_ok("-5"));
  EXPECT_TRUE(dom_ok("3.14"));
  EXPECT_TRUE(dom_ok("1.5e2"));
  EXPECT_TRUE(dom_ok("-0.5"));
  EXPECT_TRUE(dom_ok("1e10"));
  EXPECT_TRUE(dom_ok("-1.5e-3"));
}

// Lenient scanner: accepts non-RFC number formats
TEST(StrictNumber, LenientScannerBehavior) {
  EXPECT_TRUE(dom_ok("01"));   // leading zero: accepted
  EXPECT_TRUE(dom_ok("007"));  // leading zeros: accepted
  EXPECT_TRUE(dom_ok("1."));   // trailing dot: accepted
  EXPECT_TRUE(dom_ok("1e"));   // incomplete exponent: accepted
}

// Invalid value-start chars: not in action LUT -> parse error
TEST(StrictNumber, InvalidStartCharsRejected) {
  EXPECT_FALSE(dom_ok("+123")); // '+' not in number case
  EXPECT_FALSE(dom_ok(".5"));   // '.' not in number case
}
